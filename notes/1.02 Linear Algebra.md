...
We deal with these data points - vectors - simultaneously in aggregates known as __matrices__.
...
Vectors have __magnitude__ and __direction__, e.g. 5 miles/hour going east.
...
For example: the notation for the two-dimensional real coordinate space is $\mathbb R^2$, which is
all possible real-valued 2-tuples (i.e. all 2D vectors whose components are real numbers). If we wanted to describe an arbitrary two-dimensional vector, we could do so with $\vec v \in \mathbb R^2$.
...
A __row vector__ $x \in \mathbb R^n$ has 1 row and $n$ columns.
...
The __dot product__ (also called __inner product__) of two vectors $\vec{a}, \vec{b} \in \mathbb R^n$ (that is, they must be of the same dimension) is notated:
...
The __norm__ of a vector $x \in \mathbb R^n$, denoted $||x||$, is the "length" of the vector.
...
non-zero, and their dot product is 0, that is, $\vec{a} \cdot \vec{b} = \vec{0}$, then they are perpendicular to each other.

Whenever a pair of vectors satisfies this condition $\vec{a} \cdot \vec{b} = \vec{0}$,
it is said that the two vectors are __orthogonal__ (i.e. perpendicular).
...
For __outer products__, the two vectors do not need to be of the same dimension (i.e. $x \in \mathbb R^n, y \in \mathbb R^m$), and the result is a matrix instead of a scalar:
...
for any $\mathbb R^n$. Cross products are only defined in $\mathbb R^3$.
...
multiplications of that vector (where the scalar $c$ is any real number):
...
vector, which we'll call $\vec{x}$. So we could define a line as:
...
Say you want to find the line that goes through them. First you need to find the vector
...
wouldn't really be a "line". In $\mathbb R^3$ space, for instance, this would define a plane.)
...
A __linear combination__ is just some sum of the combination of these vectors,
  scaled by arbitrary constants ($c_1 \to c_n \in \mathbb R$):
...
_Any_ vector in the space $\mathbb R^2$ can represented by some linear combination of
these two vectors.
...
does _not_ represent all the vectors in $\mathbb R^2$ :
...
You can think of a linearly dependent set as one that contains a redundant vector -
...
Thus you can calculate the zero vector as a linear combination of the vectors where at
...
describe that set's entire space (e.g. $\mathbb R^n$).
...
arbitrary vector in $\mathbb R^3$ :
...
(that is, the same number of components as there are columns in the matrix, i.e. $\vec{x} \in \mathbb R^n$):
...
Say we have set of vectors $V$ which is a subset of $\mathbb R^n$,
...
- for a vector $\vec{x}$ in $V$, $c\vec{x}$ (where $c \in \mathbb R$) must also be in
...
$$ S = \left\lbrace \begin{bmatrix} x_1 \\ x_2 \end{bmatrix} \in \mathbb R^2 \,:\, x_1 \ge
0 \right\rbrace $$
...
That is, for any vector in the space $\mathbb R^n$, multiplying the matrix by it just
...
__standard basis for $\mathbb R^n$__.
...
Given that $V$ is some subspace of $\mathbb R^n$, the __orthogonal compliment of $V$__,
notated $V^{\perp}$ :
...
vectors, that is, $B$ is a basis for $\mathbb R^n$)
...
Say we have a linear transformation $T: \mathbb R^n \to \mathbb R^n$, which we can express
...
Say we have another basis $B = \{ \vec{v_1}, \vec{v_2}, \dots, \vec{v_n} \}$ for $\mathbb R^n$,
that is, it is a basis for for $\mathbb R^n$.
...
where we can take two vectors $\vec{a}, \vec{b} \in \mathbb R^n$ and the following
...
These examples are all in $\mathbb R^2$ since it's easier to visualize. But you can scale
them up to any $\mathbb R^n$.
...
A composition of linear transformations is also a linear transformation (proof omitted
...
When you pass a set of vectors (i.e. a subset of a domain $\mathbb R^n$) through a transformation,
...
i.e. $T(\mathbb R^n)$, the terminology is different: that is called the _image of $T$_,
notated $\Img(T)$.
...
Given that $V$ is a subspace of $\mathbb R^n$ and $B = \{ \vec{v_1}, \vec{v_2}, \dots, \vec{v_k} \}$  is an
orthonormal basis for $V$. We have a vector $\vec{x} \in \mathbb R^n$, so $\vec{x} = \vec{v} + \vec{w}$ where $\vec{v} \in V$ and $\vec{w} \in V^{\perp}$.
...
$\mathbb R^n$ consisting entirely of eigenvectors for $\mathbf A$.
