...
- __non-negativity__: for all $\alpha \in \mathcal F, P(\alpha) \geq 0$.
- __trivial event__: $P(\Omega) = 1$.
- __additivity__: For all $\alpha, \beta \in \mathcal F$ and $\alpha \cap \beta = \emptyset$, $P(\alpha \cup \beta) = P(\alpha) + P(\beta)$.
...
Distributions themselves are described by __parameters__ - variables which determine the specifics of a distribution. Different kinds of distributions are described by different sets of parameters. For instance, the particular shape of a normal distribution is determined by $\mu$ (the mean) and $\sigma$ (the standard deviation).
...
TODO show some different covariance matrices for different sets of 2d data
...
(TODO move this introduction to parameters elsewhere)
